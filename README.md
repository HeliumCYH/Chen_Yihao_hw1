...
特判a列数不等于b行数报错，否则遍历进行乘法运算，利用b每一列分别乘上a即可
...
特判行数不等于列数报错，否则自行定义函数求拉普拉斯变换余下的矩阵，然后遍历第一行，利用拉普拉斯变换递归求解行列式

### 矩阵求逆

逆矩阵元素为原矩阵对应位置元素的余子式，然后对余子式矩阵求逆再除掉原矩阵的行列式即可得到矩阵的逆
...
查询最后一个不全为零的行减一即为原矩阵的秩

### 求方阵的迹

特判非方阵情况报错，遍历对角线上的元素求和即可

## 运行截图

![image-20240414233052661](C:\Users\Helium\AppData\Roaming\Typora\typora-user-images\image-20240414233052661.png)
>>>>>>> ea99bdc (create respository)
>>>>>>> f401d57 (Test)
